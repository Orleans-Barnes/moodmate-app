...
export function ProScreen({ navigation }: Props) {
  ...
  // The backend allows exactly one subscription row per account ever (trial or paid) - once
  // status is non-null, calling /subscription/trial again 409s. So the free-trial CTA only makes
  // sense pre-first-use; everything after that is a status display, not an offer.
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      {!subscription.pro && !neverSubscribed && (
        <Card tint="sun" style={styles.statusCard}>
          <Text style={styles.statusText}>
            Your subscription has lapsed. Paid checkout isn&apos;t available yet — coming soon.
          </Text>
        </Card>
      )}
      ...
      {neverSubscribed && (monthly || yearly) && (
        <>
          ...
          <Button
            ...
            variant="primary"
            fullWidth
            disabled={starting || !activeCode}
            onPress={handleStartTrial}
          />
          <Text style={styles.fineprint}>No payment needed for the trial · paid checkout coming soon</Text>
        </>
      )}
    </ScrollView>
  );
