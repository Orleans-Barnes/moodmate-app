    ...
    <Pressable style={s.card} onPress={() => setOpen((v) => !v)}>
      <View style={s.cardHeader}>
        <View style={s.iconWrap}>
          <Ionicons name={icon} size={20} color={colors.lavender} />
        </View>
                  ...
                  onPress: async () => {
                    ...
                    try {
                      const res = await fetch(
                        'http://10.253.17.149:8080/api/users/me',
                        { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } },
                      );
                      if (!res.ok && res.status !== 204) {
                        throw new Error('Server error — please contact support@moodmate.app');
                      }
                      await logout();
                      Alert.alert(
                        'Account deleted',
    ...
    <View style={[s.root, { paddingTop: insets.top }]}>
      ...
      >
        ...
        <View style={s.heroCard}>
          <Ionicons name="shield-checkmark" size={36} color={colors.lavender} />
        ...
        <View style={s.rightsCard}>
          {[
            { icon: 'download-outline' as const, text: 'Request a copy of your data — email privacy@moodmate.app' },
            ...
            <View key={text} style={s.rightRow}>
              <Ionicons name={icon} size={16} color={colors.lavender} style={{ marginTop: 2 }} />
        ...
        {/* Danger zone */}
        <Text style={[s.sectionLabel, { color: '#DC2626' }]}>Danger zone</Text>
        ...
        <Text style={s.footer}>
          ...
          <Text style={s.footerLink}>privacy@moodmate.app</Text>
        </Text>
      </ScrollView>
    </View>
  );
...
const s = StyleSheet.create({
  ...
  dangerInfo: { gap: 4 },
  dangerTitle: { fontFamily: fonts.bodyBold, fontSize: fontSizes.md, color: '#DC2626' },
  dangerBody: { fontFamily: fonts.body, fontSize: fontSizes.sm, color: '#7F1D1D', lineHeight: 20 },
  deleteBtn: {
    backgroundColor: '#DC2626', borderRadius: radii.md,
  ...
  footerLink: { color: colors.lavender, fontFamily: fonts.bodyMedium },
});
