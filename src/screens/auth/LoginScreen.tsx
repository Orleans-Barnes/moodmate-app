...
export function LoginScreen({ navigation, route }: Props) {
  ...
  const handleLogin = async () => {
    ...
    setLoading(true);
    try {
      const { token, refreshToken, user } = await login({ email: email.trim().toLowerCase(), password });
        ...
        <Animated.View style={[s.footerGroup, slide(footAnim, 20)]}>
          ...
          {role === 'STUDENT' && (
            <Pressable
              ...
              accessibilityHint="Explore MoodMate without creating an account. Some features will be limited."
            >
...
const s = StyleSheet.create({
  ...
  guestLink: { paddingVertical: 4, paddingHorizontal: 16 },
  guestText: { fontFamily: fonts.bodyMedium, fontSize: fontSizes.xs, color: colors.inkFaint, textAlign: 'center' },
  footerNote: {
    fontFamily: fonts.bodyMedium,
    fontSize: fontSizes.xs,
