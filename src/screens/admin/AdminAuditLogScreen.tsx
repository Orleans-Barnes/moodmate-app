    ...
    <View style={[s.root, { paddingTop: insets.top }]}>
      ...
      {error ? (
        ...
        <FlatList
          ...
          ListFooterComponent={loadingMore ? <ActivityIndicator color={colors.lavender} style={{ marginVertical: 16 }} /> : null}
          renderItem={({ item }) => (
            <View style={s.logRow}>
              <View style={s.logIcon}>
                <Ionicons name={ACTION_ICONS[item.action] ?? 'ellipse-outline'} size={16} color={colors.lavenderDeep} />
              </View>
              <View style={{ flex: 1 }}>
...
const s = StyleSheet.create({
  ...
  logIcon: {
    width: 30, height: 30, borderRadius: 15, backgroundColor: colors.lavenderSoft,
