    ...
    <View style={[s.root, { paddingTop: insets.top }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: insets.bottom + 40 }}>
        ...
        {error ? (
          ...
          <ActivityIndicator color={colors.lavender} style={{ marginVertical: 24 }} />
        ) : flags.length === 0 ? (
          ...
          flags.map((flag) => (
            <View key={flag.id} style={s.flagRow}>
              ...
              {togglingId === flag.id
                ? <ActivityIndicator size="small" color={colors.lavender} style={{ marginHorizontal: 12 }} />
                : <Switch value={flag.enabled} onValueChange={() => handleToggle(flag)} trackColor={{ true: colors.lavender }} />
              }
...
const s = StyleSheet.create({
  ...
  audienceChip: { paddingHorizontal: 10, paddingVertical: 6, borderRadius: radii.pill, backgroundColor: '#F4F2F8' },
  audienceChipActive: { backgroundColor: colors.lavender },
  audienceChipText: { fontFamily: fonts.bodyBold, fontSize: 10, color: colors.inkSoft },
  audienceChipTextActive: { color: '#fff' },

  primaryBtn: {
    marginTop: 4, backgroundColor: colors.lavender, borderRadius: radii.md,
