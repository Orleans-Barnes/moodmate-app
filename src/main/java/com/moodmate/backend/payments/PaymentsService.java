 ...
 *
 * Cross-domain calls:
 *  - User email for Paystack → via {@link UserLookupApi} (no auth package import)
 *  - Leaf credit after purchase → via {@link WalletInternalApi} (no wallet/wellness imports)
 */
@Service
...
public class PaymentsService {
    ...
    public CheckoutResponse checkoutSubscription(Long userId, String planCode) {
        SubscriptionPlan plan = subscriptionPlanRepository.findByCode(planCode)
                .orElseThrow(() -> new ResourceNotFoundException("Unknown subscription plan: " + planCode));
        String userEmail = userLookupApi.getUserEmail(userId);
        ...
        PaystackInitializeResponse.Data data = paystackClient.initializeTransaction(
                userEmail, plan.getPricePesewas(), reference,
    ...
    public CheckoutResponse checkoutLeafPack(Long userId, String packCode) {
        ...
        PaystackInitializeResponse.Data data = paystackClient.initializeTransaction(
                userEmail, pack.getPricePesewas(), reference,
    ...
    /**
     * Credits the user's leaf wallet after a successful leaf-pack purchase.
     * Delegates entirely to {@link WalletInternalApi} — payments no longer touches
     * wellness or wallet repositories directly.
     */
    private void creditLeafPack(PaymentTransaction tx) {
        ...
        walletInternalApi.creditLeaves(tx.getUserId(), pack.getLeaves(), "LEAF_PACK_PURCHASE");
    }
