...
public class JournalService {
    ...
    public JournalEntryResponse create(Long userId, JournalEntryRequest request) {
        ...
        JournalEntryResponse saved = toResponse(journalEntryRepository.save(entry));

        // Scan journal body for crisis language
        String scanText = (request.title() != null ? request.title() + " " : "") + (request.body() != null ? request.body() : "");
        crisisAlertService.analyseAndFlag(userId, scanText, CrisisSource.JOURNAL);
