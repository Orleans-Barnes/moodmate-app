...
public class SupportService {
    ...
    public CounsellorRequestResponse requestCounsellorStatus(Long userId, CounsellorRequestInput request) {
        ...
        UserSummary requester = userLookupApi.getUserSummary(userId);
    ...
    public List<CounsellorAppointmentView> listCounsellorAppointments(Long counsellorUserId) {
        ...
        Map<Long, UserSummary> students = userLookupApi.getUserSummaries(
    ...
    public CounsellorAppointmentView confirmAppointment(Long counsellorUserId, Long appointmentId) {
        ...
        appointment = appointmentRepository.save(appointment);
        return toCounsellorView(appointment, userLookupApi.getUserSummaries(List.of(appointment.getUserId())));
    }
    ...
    public MessageResponse sendMessage(Long userId, Long conversationId, SendMessageRequest request) {
        ...
        MessageResponse saved = toMessageResponse(supportMessageRepository.save(message));

        // Notify the counsellor in the background (fire-and-forget)
        if (conversation.getCounsellorId() != null) {
            counsellorRepository.findById(conversation.getCounsellorId()).ifPresent(counsellor -> {
                String senderName = userLookupApi.getUserSummary(userId).fullName();
                String preview = request.body().length() > 60 ? request.body().substring(0, 57) + "…" : request.body();
                pushNotificationService.sendToUser(counsellor.getUserId(), senderName, preview,
    ...
    public MessageResponse sendCounsellorMessage(Long counsellorUserId, Long conversationId, SendMessageRequest request) {
        ...
        // Notify the student in the background (fire-and-forget)
        Counsellor counsellor = findLinkedCounsellor(counsellorUserId);
        String preview = request.body().length() > 60 ? request.body().substring(0, 57) + "…" : request.body();
        pushNotificationService.sendToUser(conversation.getUserId(), counsellor.getName(), preview,
                Map.of("type", "new_message", "conversationId", conversationId));
    ...
    private ConversationResponse toConversationResponse(Conversation conversation) {
        ...
        SupportMessage lastMsg = supportMessageRepository
                .findTopByConversationIdOrderByCreatedAtDesc(conversation.getId()).orElse(null);
        String preview = lastMsg != null ? lastMsg.getBody() : null;
        Instant lastMessageAt = lastMsg != null ? lastMsg.getCreatedAt() : null;
        ...
        return new ConversationResponse(conversation.getId(), conversation.getCounsellorId(), counsellorName,
                conversation.getPeerMentorId(), mentorName, conversation.getCreatedAt(), preview, lastMessageAt,
                unreadCount);
    }
    ...
    private CounsellorConversationView toCounsellorConversationView(Conversation c, Map<Long, UserSummary> students) {
        ...
        SupportMessage lastMsg = supportMessageRepository
                .findTopByConversationIdOrderByCreatedAtDesc(c.getId()).orElse(null);
        String preview = lastMsg != null ? lastMsg.getBody() : null;
        Instant lastMessageAt = lastMsg != null ? lastMsg.getCreatedAt() : null;
        long unreadCount = supportMessageRepository
        ...
        return new CounsellorConversationView(c.getId(), c.getUserId(), student != null ? student.fullName() : "Student",
                c.getCreatedAt(), preview, lastMessageAt, unreadCount);
    }
