...
import { calm, accents } from '@/theme/tokens';
...
const PRESETS: Record<LiquidPreset, { blobs: Omit<BlobProps, 'durX'|'durY'|'durS'|'delay'>[] }> = {
  // Calm Forest watermark — one dominant brand green with two gentle accent
  // breaks (gratitude pink, focus amber), drawn from the same curated
  // `accents` table content uses elsewhere. Deliberately NOT an equal-weight
  // rainbow (violet/red/mint/blue/yellow/magenta) — that reads as a generic
  // decorative gradient, not this app's own identity.
  wellness: { blobs: [
    { x: W * 0.05, y: H * 0.08, size: 260, color: calm.mint,               opacity: 0.16, dx: 28, dy: 36 },
    { x: W * 0.88, y: H * 0.13, size: 200, color: accents.wellness.accent, opacity: 0.12, dx: 22, dy: 30 },
    { x: W * 0.5,  y: H * 0.32, size: 180, color: calm.mint,               opacity: 0.14, dx: 38, dy: 28 },
    { x: W * 0.1,  y: H * 0.60, size: 220, color: accents.calm.accent,     opacity: 0.10, dx: 26, dy: 32 },
    { x: W * 0.80, y: H * 0.68, size: 170, color: accents.focus.accent,    opacity: 0.10, dx: 32, dy: 38 },
    { x: W * 0.45, y: H * 0.88, size: 190, color: accents.gratitude.accent, opacity: 0.09, dx: 20, dy: 24 },
  ]},
  focus: { blobs: [
