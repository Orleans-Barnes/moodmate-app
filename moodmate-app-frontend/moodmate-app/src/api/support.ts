...
export function listBookedSlots(
  ...
  from: string,
  to: string
): Promise<BookedSlotView[]> {
  const params = new URLSearchParams({ from, to });
  return apiGet<BookedSlotView[]>(`/api/support/counsellors/${counsellorId}/booked-slots?${params.toString()}`, token);
}
...
/**
 * Self-service auto-approval — called after the peer mentor completes the academy training and
 * agrees to the T&Cs. No ADMIN role needed; the backend verifies a PENDING application exists for
 * the calling user and promotes them to MENTOR automatically.
 */
export function selfApproveMentorApplication(token: string): Promise<{ id: number; status: string }> {
  return apiPost<{ id: number; status: string }>('/api/support/mentor-applications/self-approve', undefined, token);
}
...
export interface EscalationCaseView {
  id: number;
  mentorUserId: number;
  mentorName: string;
  studentName: string;
  ...
  status: 'SUBMITTED' | 'IN_REVIEW' | 'FEEDBACK_READY';
...
/** MENTOR: submit a new case escalation */
export function submitEscalation(token: string, input: SubmitEscalationInput): Promise<EscalationCaseView> {
  return apiPost<EscalationCaseView>('/api/support/escalations', input, token);
}

/** MENTOR: list their own escalation history */
export function listMyEscalations(token: string): Promise<EscalationCaseView[]> {
  return apiGet<EscalationCaseView[]>('/api/support/escalations/mine', token);
}

/** COUNSELLOR: list all escalations from all mentors */
export function listAllEscalations(token: string): Promise<EscalationCaseView[]> {
  return apiGet<EscalationCaseView[]>('/api/support/escalations', token);
}

/** COUNSELLOR: mark a case as under review */
export function markEscalationInReview(token: string, id: number): Promise<EscalationCaseView> {
  return apiPost<EscalationCaseView>(`/api/support/escalations/${id}/review`, undefined, token);
}

/** COUNSELLOR: send feedback on an escalated case */
export function sendEscalationFeedback(token: string, id: number, feedback: string): Promise<EscalationCaseView> {
  return apiPost<EscalationCaseView>(`/api/support/escalations/${id}/feedback`, { feedback }, token);
}
...
export function getHealthPulse(token: string): Promise<HealthPulse> {
  return apiGet<HealthPulse>('/api/admin/health-pulse', token);
}

export function getCheckinAnalytics(token: string, days = 30): Promise<Record<string, unknown>[]> {
  return apiGet<Record<string, unknown>[]>(`/api/admin/analytics/checkins?days=${days}`, token);
}

export function getMoodAnalytics(token: string): Promise<Record<string, unknown>[]> {
  return apiGet<Record<string, unknown>[]>('/api/admin/analytics/moods', token);
}

export function getInstitutionAnalytics(token: string): Promise<Record<string, unknown>[]> {
  return apiGet<Record<string, unknown>[]>('/api/admin/analytics/institutions', token);
}
