...
const STAT_CARDS: {
  ...
  { key: 'totalStudents',           label: 'Students',      icon: 'school-outline',      from: '#579E65', to: '#7FB98D' },
  { key: 'totalCounsellors',        label: 'Counsellors',   icon: 'people-outline',      from: '#6FA8C7', to: '#8CBFDA' },
  { key: 'totalAppointments',       label: 'Appointments',  icon: 'calendar-outline',    from: '#24412A', to: '#468752' },
  { key: 'totalCommunityPosts',     label: 'Posts',         icon: 'chatbubbles-outline', from: '#E08A5A', to: '#E8A87F' },
];
        ...
        <html>
          <head>
            <style>
              ...
              h1 { color: #24412A; border-bottom: 2px solid #579E65; padding-bottom: 10px; }
              .stat-box { display: inline-block; width: 45%; margin: 2%; padding: 15px; background: #f4f6f5; border-radius: 8px; box-sizing: border-box; }
              .stat-val { font-size: 24px; font-weight: bold; color: #579E65; }
              .stat-lbl { font-size: 14px; color: #666; }
              table { width: 100%; margin-top: 20px; border-collapse: collapse; }
              th, td { padding: 10px; text-align: left; border-bottom: 1px solid #ddd; }
              th { background-color: #24412A; color: white; }
            </style>
          </head>
          <body>
            <h1>MoodMate Platform Executive Report</h1>
            ...
            <div>
              ...
              <div class="stat-box">
                <div class="stat-val">${stats?.totalCounsellors ?? 0}</div>
                <div class="stat-lbl">Counsellors</div>
              </div>
              <div class="stat-box">
                <div class="stat-val">${stats?.totalAppointments ?? 0}</div>
                <div class="stat-lbl">Completed Appointments</div>
              </div>
              <div class="stat-box">
                <div class="stat-val">${stats?.totalCommunityPosts ?? 0}</div>
                <div class="stat-lbl">Community Forum Posts</div>
              </div>
            </div>
            <h2>System Whitelist</h2>
            <table>
              <thead>
                <tr>
                  <th>Email</th>
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              Alert.alert('Error', getErrorMessage(err, 'Could not approve. Please try again.'));
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              Alert.alert('Error', getErrorMessage(err, 'Could not reject. Please try again.'));
            } finally { setActionId(null); }
          },
        },
      ],
  ...
  return (
    <View style={[s.root, { paddingTop: insets.top }]}>
      ...
      >
        ...
        {apiError ? (
          <View style={[s.alertBanner, { borderLeftColor: colors.error, backgroundColor: colors.errorSoft, marginHorizontal: spacing.xl, marginTop: 0 }]}>
            <Ionicons name="warning-outline" size={18} color={colors.error} />
            <Text style={[s.alertText, { color: colors.errorDeep, flex: 1 }]}>{apiError}</Text>
            <Pressable onPress={() => { setLoading(true); load(); }}>
              <Text style={{ fontFamily: fonts.bodyBold, fontSize: fontSizes.sm, color: colors.error }}>Retry</Text>
            </Pressable>
          </View>
        ) : loading ? (
        ...
        {/* ── Crisis alert banner ── */}
        {openCrisisCount > 0 && (
          <View style={[s.alertBanner, { borderLeftColor: colors.error, backgroundColor: colors.errorSoft }]}>
            <Ionicons name="warning" size={18} color={colors.error} />
            <Text style={[s.alertText, { color: colors.errorDeep }]}>
        ...
        <View style={s.quickActionRow}>
          <Pressable style={s.quickActionCard} onPress={() => navigation.navigate('AdminUserManagement')}>
            <Ionicons name="people-circle-outline" size={26} color={colors.coral} />
          ...
          <Pressable style={s.quickActionCard} onPress={() => navigation.navigate('AdminCounsellorMentorManagement')}>
            <Ionicons name="medkit-outline" size={26} color={colors.coral} />
          ...
          <Pressable style={s.quickActionCard} onPress={() => navigation.navigate('AdminWellnessContent')}>
            <Ionicons name="leaf-outline" size={26} color={colors.coral} />
        ...
        <View style={[s.quickActionRow, { marginTop: spacing.sm }]}>
          ...
          <Pressable style={s.quickActionCard} onPress={() => navigation.navigate('AdminSystemSettings')}>
            <Ionicons name="settings-outline" size={26} color={colors.coral} />
            <Text style={s.quickActionText}>System{'\n'}Settings</Text>
          </Pressable>
          <Pressable style={s.quickActionCard} onPress={() => navigation.navigate('AdminAuditLog')}>
            <Ionicons name="document-text-outline" size={26} color={colors.coral} />
        ...
        <View style={[s.quickActionRow, { marginTop: spacing.sm }]}>
          <Pressable style={s.quickActionCard} onPress={() => navigation.navigate('AdminInstitutionManagement')}>
            <Ionicons name="school-outline" size={26} color={colors.coral} />
          ...
          <Pressable style={s.quickActionCard} onPress={() => navigation.navigate('AdminRevenue')}>
            <Ionicons name="cash-outline" size={26} color={colors.coral} />
            <Text style={s.quickActionText}>Revenue</Text>
          </Pressable>
          <Pressable style={s.quickActionCard} onPress={generatePdfReport}>
            <Ionicons name="document-text-outline" size={26} color={colors.coral} />
            <Text style={s.quickActionText}>Export PDF{'\n'}Report</Text>
          </Pressable>
        </View>
        ...
        {loading ? (
          <ActivityIndicator color={colors.coral} style={{ margin: 20 }} />
        ) : requests.length === 0 ? (
          ...
          requests.map((item) => (
            <View key={item.id} style={s.card}>
              <View style={s.cardHeader}>
                <LinearGradient colors={['#579E65', '#7FB98D']}
        ...
        {loading ? (
          ...
          mentorRequests.map((item) => (
            <View key={item.id} style={s.card}>
              <View style={s.cardHeader}>
                <LinearGradient colors={['#468752', '#579E65']}
                  style={s.avatarCircle}>
                  <Text style={s.avatarText}>{item.name.charAt(0).toUpperCase()}</Text>
                </LinearGradient>
                <View style={s.cardInfo}>
...
const s = StyleSheet.create({
  ...
  alertText: { fontFamily: fonts.bodyMedium, fontSize: fontSizes.sm, color: colors.sunText, flex: 1 },
  ...
  pendingBadge: { backgroundColor: colors.warningSoft, borderRadius: radii.pill, paddingHorizontal: spacing.sm, paddingVertical: 2 },
  pendingBadgeText: { fontFamily: fonts.bodyBold, fontSize: 9, color: colors.sunText, letterSpacing: 0.5 },
  ...
  infoBox: {
    ...
    backgroundColor: '#EEF3EF', borderRadius: radii.md, padding: spacing.md,
  },
  ...
  wlInput: {
    ...
    backgroundColor: colors.surfaceContainer, borderRadius: radii.sm, paddingHorizontal: spacing.md,
  ...
  wlAddBtn: {
    marginTop: spacing.sm, backgroundColor: colors.coral, borderRadius: radii.md,
