...
const CONTACT_OPTIONS = [
  {
    ...
    label: 'Email support',
    sub: 'orleansbarnes9@gmail.com',
    action: () => Linking.openURL('mailto:orleansbarnes9@gmail.com?subject=MoodMate%20Support'),
  },
  {
  ...
  return (
    <Pressable style={s.faqCard} onPress={() => setOpen((v) => !v)}>
      <View style={s.faqHeader}>
        ...
        <Ionicons name={open ? 'remove-circle-outline' : 'add-circle-outline'} size={20} color={colors.coral} />
      </View>
  ...
  return (
    <View style={s.root}>
      <View style={s.headerWrap}>
        <ScreenHeader title="Help & Support" onClose={() => navigation.goBack()} compact />
      </View>
      ...
      >
        ...
        <View style={s.heroCard}>
          <Ionicons name="help-buoy-outline" size={36} color={colors.coral} />
          ...
          {CONTACT_OPTIONS.map((opt) => (
            <Pressable key={opt.label} style={s.contactCard} onPress={opt.action}>
              <View style={s.contactIcon}>
                <Ionicons name={opt.icon} size={22} color={colors.coral} />
              </View>
