...
import { listCounsellorConversations } from '@/api/support';
...
type Props = CompositeScreenProps<
  BottomTabScreenProps<CounsellorTabParamList, 'Conversations'>,
...
export function CounsellorConversationsScreen({ navigation }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      setConvos(await listCounsellorConversations(token));
    } catch (err) {
        ...
        <View style={s.headerRow}>
          <View>
            ...
            <Text style={s.headerSub}>
              {convos.length} clients · {unreadTotal > 0 ? `${unreadTotal} unread` : 'all read'}
            </Text>
          </View>
        </View>
        <View style={s.searchBar}>
          <Ionicons name="search-outline" size={14} color="rgba(255,255,255,0.85)" />
          <TextInput
            style={s.searchInput}
      ...
      >
        {loading && convos.length === 0 ? (
          ...
          <View style={s.emptyBox}>
            <Ionicons name={query ? 'search-outline' : 'chatbubbles-outline'} size={40} color={colors.inkFaint} />
            <Text style={s.emptyTitle}>{query ? 'No results' : 'No conversations yet'}</Text>
            <Text style={s.emptySub}>{query ? 'Try a different name' : 'When students message you, they appear here'}</Text>
          </View>
        ) : (
          visible.map(c => {
...
const s = StyleSheet.create({
  ...
  avatar: { width: 50, height: 50, borderRadius: 16, backgroundColor: colors.coralDeep, alignItems: 'center', justifyContent: 'center' },
  ...
  badge: { width: 22, height: 22, borderRadius: 11, backgroundColor: colors.coralDeep, alignItems: 'center', justifyContent: 'center' },
