...
import { REACTION_ICONS } from '@/theme/iconMap';
...
const ALL_REACTIONS: { type: ReactionType; icon: string }[] = [
  { type: 'HEART', icon: REACTION_ICONS.HEART },
  { type: 'PRAYER', icon: REACTION_ICONS.PRAYER },
  { type: 'MUSCLE', icon: REACTION_ICONS.MUSCLE },
  { type: 'PARTY', icon: REACTION_ICONS.PARTY },
];
...
function fromApi(post: CommunityPostView): CommunityPost {
  ...
  return {
    ...
    reactions: ALL_REACTIONS.map(({ type, icon }) => {
      const summary = byType.get(type);
      return { type, icon, count: summary?.count ?? 0, on: summary?.reactedByMe ?? false };
    }),
  };
}
    ...
    set({
      posts: get().posts.map((post) =>
        post.id === postId
          ? {
              ...post,
              reactions: ALL_REACTIONS.map(({ type: t, icon }) => {
                const summary = byType.get(t);
                return { type: t, icon, count: summary?.count ?? 0, on: summary?.reactedByMe ?? false };
              }),
            }
          : post
      ),
