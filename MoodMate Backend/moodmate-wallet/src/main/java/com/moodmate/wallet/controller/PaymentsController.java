...
public class PaymentsController {
    ...
    @GetMapping("/books")
    public List<BookDto> books(@RequestHeader("X-User-Id") Long userId) {
        return paymentsService.listBooks(userId);
    }

    @PostMapping("/books/checkout")
    public CheckoutResponse checkoutBook(@RequestHeader("X-User-Id") Long userId,
                                          @Valid @RequestBody BookCheckoutRequest request) {
        return paymentsService.checkoutBook(userId, request.bookCode());
    }
