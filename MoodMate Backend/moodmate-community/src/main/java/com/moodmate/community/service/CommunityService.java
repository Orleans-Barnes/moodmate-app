...
public class CommunityService {
    ...
    public PostResponse createPost(Long authorId, CreatePostRequest request) {
        ...
        return toResponse(post, List.of(), authorId, 0L);
    }
    ...
    public Page<PostResponse> feed(Long viewerId, String topic, Pageable pageable) {
        ...
        Map<Long, Long> commentCountByPost = postIds.isEmpty()
                ? Map.of()
                : postCommentRepository.countByPostIdIn(postIds).stream()
                        .collect(Collectors.toMap(PostCommentRepository.PostIdCount::getPostId,
                                PostCommentRepository.PostIdCount::getTotal));
    ...
    /** Author can delete their own post; ADMIN can delete any (moderation) - same authorization
     * shape as deleteComment above. */
    @Transactional
    public void removePost(Long userId, String role, Long postId) {
        CommunityPost post = findPost(postId);
        boolean isAuthor = post.getAuthorId().equals(userId);
        boolean isAdmin = "ADMIN".equals(role);
        if (!isAuthor && !isAdmin) {
            throw new ApiException("You can only delete your own post", HttpStatus.FORBIDDEN);
        }
    ...
    private PostResponse toResponse(CommunityPost post, List<PostReaction> reactions, Long viewerId, long commentCount) {
        boolean isOwnPost = viewerId != null && viewerId.equals(post.getAuthorId());
        return new PostResponse(post.getId(), post.getAnonymousHandle(), post.getTopic(), post.getContent(),
                post.getCreatedAt(), summarize(reactions, viewerId), reactions.size(), isOwnPost, commentCount);
    }
