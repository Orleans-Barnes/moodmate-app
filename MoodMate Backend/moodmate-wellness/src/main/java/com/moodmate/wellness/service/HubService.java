...
import com.moodmate.wellness.client.NotificationsServiceClient;
    ...
    public EventResponse rsvp(Long userId, Long eventId) {
        ...
        if (!alreadyRsvped) {
            ...
            eventRsvpRepository.save(EventRsvp.builder()
                    ...
                    .build());
                    notificationsServiceClient.notify(userId, "EVENT_REMINDER", "You're registered",
                        "You're registered for " + event.getTitle() + ". We'll remind you when it starts.",
                        null, eventId);
                    notificationsServiceClient.notify(userId, "EVENT_REMINDER", "Event reminder: " + event.getTitle(),
                        event.getLocation() == null ? "Your MoodMate event starts now."
                            : "Your MoodMate event starts now at " + event.getLocation() + ".",
                        event.getStartsAt(), eventId);
        }
