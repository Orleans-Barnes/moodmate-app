...
public class AuthServiceClient {
    ...
    // Counsellor/Peer Mentor account-status gating fix - mirrors promoteToCounsellor/
    // promoteToMentor exactly, but back down to STUDENT. Called when an admin suspends a
    // counsellor or deactivates a peer mentor: without this, the account keeps its COUNSELLOR/
    // MENTOR role and full access to CounsellorTabs/MentorTabs even though the admin action was
    // meant to revoke that (suspendCounsellor/setPeerMentorActive previously only changed the
    // roster row's own status, never touched the underlying account role - a real access-control
    // gap this closes).
    public void demoteToStudent(Long userId) {
        try {
            restClient().post()
                    .uri("/internal/users/{id}/role", userId)
                    .body(new RoleUpdateRequest("STUDENT"))
                    .retrieve()
                    .toBodilessEntity();
        } catch (RestClientException e) {
            throw new ApiException("Could not reach auth-service to demote user " + userId + " to STUDENT: " + e.getMessage(),
                    HttpStatus.BAD_GATEWAY);
        }
    }
