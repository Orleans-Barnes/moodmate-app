...
public class AiChatService {
    ...
    private static final String CRISIS_REPLY = "I may not be the best person to help with this situation. "
            + "Please contact a MoodMate counsellor or peer mentor now, and open SOS for immediate support. "
            + "If you may act on these thoughts or cannot stay safe, contact local emergency services or go to the nearest emergency department now.";
    ...
    public AiChatResponse sendMessage(Long userId, AiChatRequest request) {
        ...
        if (detection != null) {
            ...
            // Do not send crisis disclosures to the language model. The safety handoff is
            // deterministic so the user is directed to qualified human help even when the model
            // is unavailable or produces an unsafe/ambiguous answer.
            repository.save(AiChatMessage.builder()
                ...
                .build());
            AiChatMessage crisisMessage = repository.save(AiChatMessage.builder()
                .userId(userId)
                .role(ChatRole.ASSISTANT)
                .content(CRISIS_REPLY)
                .messageType(ChatMessageType.TEXT)
                .build());
            return new AiChatResponse(crisisMessage.getId(), crisisMessage.getContent(), null,
                crisisMessage.getCreatedAt());
        }
